...
--          Based on lockmon, now shows 10g v$session blocking session info.
--          
--  REQUIREMENTS:
--          Version 10g.
--
...
SELECT se.username,
       ...
       lo.ctime ctime,
       DECODE(lo.block,
         0, DECODE(se.blocking_session,NULL,'No Block',se.blocking_session),
         1, 'Blocking',
         2, 'Global',
...
SELECT se.username,
       ...
       DECODE(lo.block,
         ...
         TO_CHAR(lo.block)) blkothr,
       'SYS' owner,
       DECODE(ro.name,NULL,'USN: '||TO_CHAR(tr.xidusn),ro.name) image,
       TO_CHAR( DECODE(se.row_wait_obj#,-1,' ',se.row_wait_block#) ) lblock,
       TO_CHAR( DECODE(se.row_wait_obj#,-1,' ',se.row_wait_row#)) lrow
  FROM gv$lock lo,
       gv$session se,
